---
name: new-patch
description: Adopt a new game Patch end to end, from the Typings to the pull request. Use when handed a new game Patch, a jass-history tag, or a "New Patch detected" issue.
---
...
3. **Curate.** Write one Overlay entry per missing Native at the path its item prints, by the Curation rules; fix the other error items as loop step 2 says. Done when every error item of the failure list has its entry or fix; step 4 proves it.
...
5. **Review the generated diff.** Compare the new output with the old: `git diff -- packages/reforged-types/<Game version> packages/reforged-types/<Game version>.d.ts packages/reforged-types/async-natives.json` when the new Build keeps the Game version; when it opens a new one, `git diff --no-index packages/reforged-types/<old Game version> packages/reforged-types/<new Game version>` and `git diff -- packages/reforged-types/async-natives.json`. Find every signature change (a parameter, a return type, a nullability, an async flag), removal and hierarchy change (the `extends` of a handle type). Record each in a list, the record, as **breaking** when code that compiled against the old Typings may no longer compile, else **additive**; the changesets and the pull request carry it. Done when every changed declaration of the diff is on the record with its verdict.
6. **Cover the new Natives.** This step applies once the Wrapper coverage report lands (#163), when the root `package.json` has `coverage:report`; until then, say in the pull request that it waits for #163. Run `pnpm coverage:report`. Wrap each new Native a Wrapper owns with the `add-wrapper` skill, which lands with #202 (until it does, by the README's "Rules for library code" and the doc comment of `Handle` in `packages/reforged-ts/src/handles/handle.ts`), or exclude it in the report's exclusions with a reason naming its tracking issue (`gh issue create` when none exists). Done when the report lists none of the new Natives as missing.
7. **Bump `reforged.patch`.** Move the fields and the rest of the Patch metadata as loop step 5 says, and each field the list "Which fields move" of "Choosing the bump for a Patch" names. Done when `pnpm release:check-patches` exits 0 naming the new Build for `reforged-types` and `reforged-ts`, and every item of loop step 5 is done.
8. **Write the changesets.** Give each package its bump by "Choosing the bump for a Patch" of `docs/release.md`, from the record of step 5 and the Wrappers of step 6, and name each breaking change in the text. Add them without the prompt ("Adding a changeset"). Done when the changesets give each package the bump the rules set and name every breaking change of the record.
9. **Check and commit.** Run `pnpm check` and `pnpm --filter reforged-types verify`, then commit as loop step 7 says and run `pnpm release:check-changeset`, which reads the committed changes. Done when all three exit 0 (`pnpm check` runs `typings:check`) and `git status` is clean.
