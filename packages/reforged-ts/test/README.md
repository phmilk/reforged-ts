...
Stub helpers the tests call (`__stub_fire_trigger`, `__stub_fire_timer`, `__stub_record`, `__stub_deliver_sync`, `__stub_sync_packets`, `__stub_set_clock`, `__stub_preload_file`, and the test controls `__stub_set_local_player`, `__stub_local_player`, `__stub_printed`, `__stub_displayed`, `__stub_init_globals`, `__stub_dispatch_damage` and `__stub_args` that the [reforged-test README](../../reforged-test/README.md) describes under Test controls) are declared in `stubs.d.ts`, with `StubSyncPacket`, the `{ prefix, data, from }` packet the sync helpers record and deliver, and with `StubContext`, the firing context `__stub_fire_trigger` takes: keyed by response Native name, each value typed as that Native returns it (`{ GetTriggerUnit: unit.handle, GetEventDamage: 25 }`), so a misspelt Native or a wrong value is a compile error.
...
The sync and host tests are the reference for stubbing a Native that fires an event with context, and a later Wrapper whose event carries data from another client copies them. The pieces, all in reforged-test: a send stub that records the call and keeps what was sent (`BlzSendSyncData`, read back with `__stub_sync_packets()`); a registration stub that remembers its trigger and what it filters on (`BlzTriggerRegisterPlayerSyncEvent`, the one registration that keeps state); and a delivery helper that fires the matching triggers with the response Natives answering from the context (`__stub_deliver_sync`, over `__stub_with_context`). `sync.test.ts` drives them on one copy of the library (a short and a chunked request, packets delivered shuffled, a foreign prefix and a corrupt header, a timeout fired with `__stub_fire_timer`, a network failure with `withNative`); `sync-clients.test.ts` and `host-clients.test.ts` deliver each client's packets to every client with `simulateClients`, and `host-clients.test.ts` also makes slot 2 a playing user and fires a leave event with `__stub_fire_trigger`.
...
`node/renames.test.ts` validates `migration/renames.json`, the removed and renamed symbols the migration guide and the legacy-names lint rule read, through the rename map module the workspace's scripts share, `release/src/rename-map.ts` (the loader, the symbol parser and the declaration resolver; the library imports its source by path, since a published package cannot depend on the private release package): the file must match `migration/renames.schema.json`, every entry must carry the first release's version pair (`w3ts@3` to `reforged-ts@1`), no `old` symbol may appear twice, every replacement in `new` must be, in the library's emitted declarations, a class, a public static or instance member of one, or a public member of the type of an exported value (`Init.onGlobals`, `Reforged.configure`), and every member a build step removes must have an entry. An `old` symbol is a class, a member, a bare function or enum, or an entry point of the deprecated alias as its `W3TS_HOOK` value reads (`main::before`, kind `entryPoint`), or the package itself (`w3ts`, kind `package`, whose `new` is the package name `reforged-ts` and is not checked against the declarations). A step that removes or renames a public symbol appends its entries to the map and its list of removed members to the test (`REMOVED_IN_STEP_3` to `REMOVED_IN_STEP_6`), and its behaviour changes that are not renames to `migration/behaviour-changes.md`. A version pair whose major renames nothing carries the no-renames marker instead of entries (`{ "kind": "noRenames", "versions", "note" }`, never both for one pair); `loadRenameMap` keeps the markers and `renameEntries` leaves them out.
