...
const ARCHIVE = fileURLToPath(
  new URL("./fixtures/jass-history/", import.meta.url),
);
  ...
  const fetcher = async (
    ...
    if (url.startsWith(API)) {
      ...
      const tags = JSON.parse(
        await readFile(join(ARCHIVE, "tags.json"), "utf8"),
      ) as Record<string, string>;
      const commit = tags[decodeURIComponent(url.slice(API.length))];
      if (!commit)
        throw new Error(`GET ${url} failed: 422 Unprocessable Entity`);
      return new TextEncoder().encode(commit);
    }
    if (url.startsWith(RAW)) {
      return new Uint8Array(
        await readFile(join(ARCHIVE, url.slice(RAW.length))),
      );
    }
...
const fixtureFile = (name: string) =>
  readFile(join(ARCHIVE, COMMIT, "timeline/scripts", name));
...
describe("patchFromTag", () => {
  it("reads the Build out of a jass-history tag", () => {
    expect(patchFromTag("Reforged-v3.0.0.24268-w3-3a9d8f2")).toBe(
      "3.0.0.24268",
    );
    expect(patchFromTag("Reforged-v3.0.0.24277-w3t-e38e03b")).toBe(
      "3.0.0.24277",
    );
  });

  it("rejects anything that is not a Reforged tag", () => {
    expect(() => patchFromTag("v1.26")).toThrow(
      /not a jass-history Reforged tag/,
    );
    expect(() => patchFromTag("Reforged-v3.0.0")).toThrow(
      /not a jass-history Reforged tag/,
    );
  });
});
...
`,
  ...
  it("stores the files byte for byte, CRLF included, and nothing else from the upstream folder", async () => {
    ...
    expect((await readdir(patchDir)).sort()).toEqual([
      "blizzard.j",
      "common.ai",
      "common.j",
      "provenance.json",
    ...
    for (const name of ["common.j", "blizzard.j", "common.ai"]) {
      expect(
        (await readFile(join(patchDir, name))).equals(await fixtureFile(name)),
      ).toBe(true);
    }
    expect(
      (await readFile(join(patchDir, "common.ai"), "latin1")).includes("\r\n"),
    ).toBe(true);
  });

  it("leaves a folder byte for byte as it was when the same tag is vendored again on a later day", async () => {
    const first = await vendorTag({
      tag: TAG,
      vendorRoot,
      fetcher: archiveFetcher(),
      now: NOW,
    ...
    expect(
      (await readFile(join(again.patchDir, "provenance.json"))).equals(before),
    ).toBe(true);
  });
  ...
  it("records the new download date when the stored provenance differs", async () => {
    ...
    await writeFile(
      file,
      (await readFile(file, "utf8")).replace(/"bytes": \d+/, '"bytes": 1'),
    );
    ...
    expect(parseProvenance(await readFile(file, "utf8")).downloaded).toBe(
      "2027-03-01",
    );
  });
  ...
  it("fails on an unknown tag and writes nothing", async () => {
    await expect(
      vendorTag({
        tag: "Reforged-v1.2.3.4-w3-missing",
        vendorRoot,
        fetcher: archiveFetcher(),
        now: NOW,
      }),
...
describe("verify", () => {
  ...
  it("fails on a tampered file", async () => {
    ...
    expect(problems[0]).toMatch(
      /^blizzard\.j: sha256 [0-9a-f]{64} does not match recorded 3056ce29/,
    );
  ...
  it("fails on a missing or malformed provenance file", async () => {
    ...
    await expect(verifyPatchDir(patchDir)).rejects.toThrow(
