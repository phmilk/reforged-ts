...
`HandleMap` and `HandleSet`, the runtime Guards of `reforged-ts`, drop an entry when its object is destroyed. They ship with the runtime Guards (#52); until then the rule names them in its message and suggestion only. `WeakMap` and `WeakSet` are not reported: they do not keep their keys alive.
