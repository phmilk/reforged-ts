...
- a call to a Native, or to a member of a `reforged-ts` Wrapper (an accessor assignment such as `unit.life = 0` counts as a call to its setter), that `data/local-safe.json` does not list. The file lists the calls that only change what the local player sees or hears: frame setters, vertex colours, the camera, sounds and music, the display Natives, `print`, and their Wrapper members. A Native that only reads (`GetUnitX`, `BlzGetFrameByName`) is reported too: the rule cannot tell a read from a handle allocation, and some reads allocate one;
